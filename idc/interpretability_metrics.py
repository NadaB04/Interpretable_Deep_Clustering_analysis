...
def get_accuracy(cluster_assignments, y_true, n_clusters):
    ...
    n_clusters = max(n_clusters,
                     int(np.array(cluster_assignments).max()) + 1,
                     int(np.array(y_true).max()) + 1)
    confusion_mat = confusion_matrix(y_true, cluster_assignments, labels=list(range(n_clusters)))
