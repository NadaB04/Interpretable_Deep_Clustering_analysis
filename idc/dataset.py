...
class BIASE(ClusteringDataset):
    ...
    def remove_zero_columns(X):
        return remove_zero_columns(X)

    @staticmethod
    def transform(X):
        # Normalize each cell (row) by its total library size, then scale to median
...
class MINIBOONE(ClusteringDataset):
    def __init__(self, data, targets):
        super().__init__(data, targets)
    ...
    def setup(cls, cfg):
        scaler = getattr(preprocessing, cfg.scaler)()
        data = np.loadtxt(f"{cfg.data_dir}/miniboone_numpy.txt")
        X = data[:, :-1]
        Y = data[:, -1]
        Y = Y - Y.min()
        X = scaler.fit_transform(X)
...
class ALBERT(ClusteringDataset):
    def __init__(self, data, targets):
        super().__init__(data, targets)
    ...
    def setup(cls, cfg):
        scaler = getattr(preprocessing, cfg.scaler)()
        data = np.loadtxt(f"{cfg.data_dir}/albert_numpy.txt")
        X = data[:, :-1]
        Y = data[:, -1]
        Y = Y - Y.min()
        X = scaler.fit_transform(X)
...
class Synthetic(Dataset):
    ...
    def setup(cls, num_samples=5000, num_features=3, num_clusters=3, num_noise_dims=10):
        ...
        bg = np.random.normal(loc=0, scale=0.1, size=(num_samples, num_noise_dims))
